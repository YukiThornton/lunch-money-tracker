...
import com.thornton.yuki.lunchmoneytracker.helper.ResourceHelper
...
class MainActivity : AppCompatActivity() {
    ...
    private fun createAndAddEntryCard(container: LinearLayout, transaction: Transaction) {
        ...
        if (transaction.type == Transaction.Type.PLUS) {
            newCard.setBackgroundColor(ResourceHelper.getThemeColor(this, R.attr.colorButtonNormal))
        }
        if (transaction.isAutoGenerated) {
            newCard.findViewById<ImageView>(R.id.repeat_icon).visibility = View.VISIBLE
        }
